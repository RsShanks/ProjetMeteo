...
//test des arguments
    ...
    while ((j = getopt (argc, argv, "f:o:c:l:t:p:whm:z:k:r:")) != -1) {
        switch (j) {
            case 'f':
                entree = optarg;
                i = strlen(entree);
                if ( entree[i-1] != 'v' || entree[i-2] != 's' || entree[i-3] != 'c')  return 2;
                nbarg += 1;
                break;
            case 'o':
                sortie = optarg;
                i = strlen(sortie);
                if ( sortie[i-1] != 'v' || sortie[i-2] != 's' || sortie[i-3] != 'c') { printf("%c", sortie[i-1]); return 3;}
                nbarg += 1;
                break;
            case 't':
                nbarg+= 1;
    ...
    {    
        ...
        else if ( arg == 51 ) a = fabricationAVLm2(ligne,b,a,nbligne,nbcolonne,fp,propre,reverse);   //mode : 2nd tri du m
